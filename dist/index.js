...
//! Functions
function withChatAction(ctx, result) {
    ctx.replyWithChatAction("typing");
    setTimeout(() => {
        result();
    }, 250);
}
function isSuperGroup(ctx, result) {
...
function isAdmin(ctx, member, result) {
...
function isNotAdmin(ctx, member, result) {
    if (!(member.status === 'creator' || member.status === 'administrator')) {
        result();
    }
    else {
...
function repliedMessageExists(ctx, result) {
